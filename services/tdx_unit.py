...
class TdxUnit:
    ...
    def format_time(time: datetime) -> str:
        """Format datetime into the format required by the TDX API"""
        return time.strftime('%Y-%m-%dT%H%%3A%M%%3A%S')
    ...
    def update_access_token(self):
        ...
        response = requests.post(auth_url, headers=self.get_auth_header(), data=self.get_auth_body())
        response_data = response.json()
        self.access_token_info['access_token'] = response_data['access_token']
        self.access_token_info['expire_time'] = datetime.now() + timedelta(seconds=response_data['expires_in'])

    def get_access_token(self) -> str:
        """Retrieve a valid access token"""
        if not self.access_token_info['access_token'] or datetime.now() >= self.access_token_info['expire_time']:
    ...
    def get_result(self, input_string: str) -> str:
        ...
        response = requests.get(self.get_url(), headers=self.get_data_header(access_token))
        response_data = response.json()

        if response_data.get('result') == 'fail':
            return json.dumps({'result': False, 'message': response_data.get('error', 'Unknown error')})
